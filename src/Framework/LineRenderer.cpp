...
LineRenderer::LineRenderer(uint a_maxLines /* = 16384 */)
    : m_maxLines(a_maxLines), m_lineCount(0), m_lines(new Line[a_maxLines]) {
  ...
  m_programID = Utility::loadShaderFromBuffer(
      2, aszInputs, 1, aszOutputs, reinterpret_cast<uchar *>(vsSource),
      reinterpret_cast<uchar *>(fsSource));
...
void LineRenderer::createBuffers() {
  ...
  glBufferData(GL_ARRAY_BUFFER, m_maxLines * sizeof(Line), m_lines,
               GL_DYNAMIC_DRAW);

  uint uiIndexCount = m_maxLines * 2;
  uint *auiIndices = new uint[uiIndexCount];
  for (uint i = 0; i < uiIndexCount; ++i) {
    auiIndices[i] = i;
  }
  ...
  glVertexAttribPointer(1, 4, GL_FLOAT, GL_TRUE, sizeof(LineVertex),
                        ((char *)0) + 16);

  glBindVertexArray(0);
}

void LineRenderer::addLine(const glm::vec4 &a_rv0, const glm::vec4 &a_rv1,
                           const glm::vec4 &a_colour) {
  addLine(a_rv0, a_rv1, a_colour, a_colour);
}

void LineRenderer::addLine(const glm::vec4 &a_rv0, const glm::vec4 &a_rv1,
                           const glm::vec4 &a_colour0,
                           const glm::vec4 &a_colour1) {
...
void LineRenderer::clear() { m_lineCount = 0; }
