...
public:
  ...
  BaseSprite(const char *a_pTexture, float a_fWidth, float a_fHeight,
             const NHTV::Vertex *a_vertexData,
             glm::vec4 a_v4colour = glm::vec4(1.f, 1.f, 1.f, 1.f));
  BaseSprite(const char *a_pTexture, const glm::vec2 &a_v2Size,
             const glm::vec2 &a_v2Origin,
             const glm::vec4 a_fv4UVCoordinates = glm::vec4(0.f, 0.f, 1.f, 1.f),
             glm::vec4 a_v4colour = glm::vec4(1.f, 1.f, 1.f, 1.f));
  BaseSprite(const BaseSprite &pSprite);
  ~BaseSprite();
  // Draw functionality
  void Draw(uint a_uiProgram);
  ///////////////////////////////////////////////////////////////////////////
  // Fucntions to set and Get sprite attributes
  ///////////////////////////////////////////////////////////////////////////
  void SetRotation(const glm::quat &a_qRotation);
  ...
  void SetSpriteID(uint a_uiSpriteID);
  void GetSpriteID(uint &a_uiSpriteID) const;

  uint GetTextureID() const { return m_uiTextureID; }

  void SetUVCoords(const glm::vec2 &a_minUVCoords,
                   const glm::vec2 &a_maxUVCoords);
  void GetUVCoords(glm::vec2 &a_minUVCoords, glm::vec2 &a_maxUVCoords) const;

  void SetBlendParameters(const uint &a_uiSourceFactor,
                          const uint &a_uiDestinationFactor);
  void GetBlendParameters(uint &a_uiSourceFactor,
...
private:
  ...
  uint m_uiTextureID;
  uint m_uSourceBlendMode;
  uint m_uDestinationBlendMode;
};
///////////////////////////////////////////////////////////////////////////
// inline functions
