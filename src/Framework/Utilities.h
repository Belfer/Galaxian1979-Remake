...
//////////////////////////////////////////////////////////////////////////
inline float Lerp(float fA, float fB, float fT) { return (fB - fA) * fT + fA; }
...
public:
  ...
  static uint loadTexture(const char *a_pFilename, int &a_uiWidth,
                          int &a_uiHeight, int &a_uiBPP);

  // loads a font in from memory into an OpenGL format
  static uint loadFont(const char *a_pFilename, int &a_iWidth, int &a_iHeight,
                       int &a_iBPP);

  // creates a shader program using the specified shader code files and a list
  // of input attributes and output attributes
  static uint loadShaderFromFile(
  ...
  // creates a shader program using the specified shader code files and a list
  // of input attributes and output attributes
  static uint loadShaderFromBuffer(
      uint a_uiInputAttributeCount, const char **a_aszInputAttributes,
      uint a_uiOutputAttributeCount, const char **a_aszOutputAttributes,
  ...
  static uchar *fileToBuffer(const char *a_szPath);

  // utilities for timing; Get() updates timers and returns time since last Get
  // call
...
private:
  static bool compileShader(const uint a_shaderHandle,
